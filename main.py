...
class Snake:
    ...
    def death_event(self):
        ...
        pyxel.stop()
        pyxel.play(0, 1)
        self.enregistre_score()
    def enregistre_score(self):
        score = self.score
        conn = sqlite3.connect("snake_scores.db")
        cursor = conn.cursor()
    ...
    def draw_score(self):
        ...
        # Increase the size of the score by drawing a filled rectangle
        pyxel.rect(1, 1, len(score) * pyxel.FONT_WIDTH, HEIGHT_SCORE, COL_SCORE)
        pyxel.text(1, 1, score, COL_BACKGROUND)
...
class Menu:
    ...
    def start(self):
        pyxel.init(256,256, title="Snake!", fps=20)
